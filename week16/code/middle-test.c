...
void *server_fun(void *n){
  struct sarg *pt=(struct sarg *)n;
	char *return_val;
	return_val=malloc(sizeof(char)*256);
	...
	sprintf(return_val, "server back!\n");
  pthread_exit((void *)return_val);
  return (void *)0;

}
void *client_fun(void *n){
  struct sarg *pt=(struct sarg *)n;
	char *return_val;
	return_val=malloc(sizeof(char)*256);
...
int main(){
	...
	ret[0]=pthread_create(&tid[0],NULL,server_fun,NULL);
	if(ret[0] != 0){
		perror("failed!\n");
		return -1;
	}
	...
	ret[1]=pthread_create(&tid[1],NULL,client_fun,NULL);
	if(ret[1] != 0){
		perror("failed!\n");
		return -1;
	}
